...
class MrFlow : public FountainFlow {
public:
    MrFlow(uint32_t id, double start_time, uint32_t size, Host *s, Host *d);
    ...
    // sender logic
    void send_rts(int iter, int round);
    void send_decision_pkt(int iter, int round, bool accept);
    void receive_ack(MRAck* p);
