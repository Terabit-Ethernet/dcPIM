...
NewRoundEvent::~NewRoundEvent() {
    if (host->new_round_evt == this) {
        host->new_round_evt = NULL;
    }
}

void NewRoundEvent::process_event() {
    this->host->new_round_evt = NULL;
    this->host->start_new_epoch(this->time, this->round);
}


ProcessReceiverIterEvent::ProcessReceiverIterEvent(double time, MrEpoch *epoch)
...
ProcessSenderIterEvent::ProcessSenderIterEvent(double time, MrEpoch *epoch)
...
// sender logic
void MrEpoch::receive_cts(MRCTS *p) {
    ...
    if(debug_flow(p->flow->id)) {
        std::cout << get_current_time() << " round " << this->round << "iter " << this->iter << " receive cts for flow " << p->flow->id << "host: " << this->host->id << std::endl; 
    }
...
void MrEpoch::advance_iter() {
    this->iter++;
    this->rts_q.clear();
    this->cts_q.clear();
}
void MrEpoch::receive_offer_packet(OfferPkt *p) {
...
void MrEpoch::receive_ctsr(CTSR *p) {
    // assert(this->iter == p->iter + 1);
    assert(p->round == this->round);
    if(debug_host(this->host->id)) {
        std::cout << get_current_time() << " round " << this->round << " iter " << this->iter << " receive ctsr packet from dst " << p->flow->dst->id << " src:" << this->host->id  << " q delay:" << p->total_queuing_delay << std::endl; 
    }
...
// receiver logic
void MrEpoch::receive_decision_pkt(DecisionPkt *p) {
    ...
    if(p->accept) {
        ...
        if(debug_host(this->host->id)) {
            std::cout << get_current_time() << " round " << this->round << " iter " << this->iter << " match src " << p->flow->src->id << "dst:" << this->host->id  << " q delay:" << p->total_queuing_delay << std::endl; 
        }
        assert(this->match_sender == NULL);
        this->match_sender = (MrHost*)p->flow->src;
        if(this->iter > params.mr_iter_limit && this->host->cur_round == this->round) {
            // corner case
...
void MrEpoch::receive_rts(MRRTS *p) {
    if(p->iter < this->iter)
        return;
    assert(p->round == this->round);
    assert(p->iter == this->iter);
    if(debug_host(this->host->id)) {
        std::cout << get_current_time() << " round " << this->round << " iter " << this->iter << " receive rts for src " << p->flow->src->id << " dst:" << this->host->id << std::endl; 
    }
    if(this->match_sender != NULL) {
...
void MrEpoch::handle_all_rts() {
    if(this->match_sender != NULL)
        return;
    assert(this->match_sender == NULL);
    uint32_t index = 0;
    if(debug_host(this->host->id)) {
        std::cout << get_current_time() << " round " << this->round << " iter " << this->iter << " handle of all rts dst:" << this->host->id << std::endl; 
    }
    ...
    for(uint32_t i = 0; i < this->cts_q.size(); i++) {
        if (i == index && this->match_receiver == NULL) {
            ...
            if(debug_host(this->cts_q[i].f->dst->id)) {
                std::cout << get_current_time() << " round " << this->round << " iter " << this->iter << " src " << this->host->id << " accept " << this->cts_q[i].f->dst->id << std::endl;
            }
            this->match_receiver = (MrHost*)(this->cts_q[i].f->dst);
            this->receiver_state[this->cts_q[i].f->dst->id] = false;
            if(this->cts_q[i].prompt && this->host->receiver == NULL) {
                assert(this->host->cur_round == this->round - 1);
...
void MrEpoch::schedule_sender_iter_evt() {
    assert(this->proc_sender_iter_evt == NULL);
    this->handle_all_cts();
    this->advance_iter();
    if(debug_host(this->host->id)) {
    ...
    if(this->iter > params.mr_iter_limit) {
        if(debug_host(this->host->id)) {
            std::cout << get_current_time() << " new round start sending packets: " << this->round  << std::endl;
        }
        this->host->receiver = this->match_receiver;
        // senders starts to send packets at this epoch
        this->host->cur_round = this->round;
        if(this->host->epochs.count(this->round - 1) > 0){
            this->host->epochs.erase(this->round - 1);
        }
        if(this->host->host_proc_event != NULL && this->host->host_proc_event->is_timeout) {
            this->host->host_proc_event->cancelled = true;
            this->host->host_proc_event = NULL;
...
void MrEpoch::schedule_receiver_iter_evt() {
...
void MrHost::start_new_epoch(double time, int round) {
    assert(this->epochs.count(round) == 0);
    if (total_finished_flows >= params.num_flows_to_run)
        return;
    this->epochs[round].round = round;
    this->epochs[round].iter = 0;
    ...
    if(debug_host(this->id)){
        std::cout << get_current_time() << "new round start " << round << std::endl;
    }
}
