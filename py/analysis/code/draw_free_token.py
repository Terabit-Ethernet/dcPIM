...
def read_outputs(direc, matric, lastFlowID, bandwidth, num_nodes, trace):
    ...
    for i in algos:
        for j in tokens:
            ...
            util[i][j] = total_sent_packets  * 1500 * 8 / (( finish_time - start_time) * bandwidth * num_nodes) / load
            fct_oct_ratio[i][j] = get_mean_fct_oct_ratio(output)
    return util, fct_oct_ratio

def draw_graph(dicts, name, ymin = 0, ymax = 1.0):
    fig, ax = plt.subplots()
    ax.set_ylim(ymin=ymin, ymax = ymax)
    t = np.array(tokens) * BDP
    values = {}
    for j in algos:
        values[j] = []
    for i in algos:
    ...
    i = 0
    for j in algos:
        eb = plt.errorbar(np.array(t), values[j], marker = marker[0], label = j, alpha = 0.5)
        # eb[-1][0].set_linestyle('--')
    print values

    ax.legend(fancybox=False, shadow=False, frameon=False, loc = 1)
    ax.set_xticks(np.array(t))
    ax.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())

    plt.xlabel('free tokens')
    plt.ylabel(name)
    ## add a legend
    plt.show()
    plt.savefig(name)
...
def main():
    ...
    util, fct_oct_ratio =  read_outputs("../../../data/free_token/" + date, "", 99999, 40000000000, 144, trace)
    draw_graph(util, trace + " Free Token Utilization")
    draw_graph(fct_oct_ratio,  trace + " Free Token FCT_OCT_ratio", 0, 30.0)
main()
