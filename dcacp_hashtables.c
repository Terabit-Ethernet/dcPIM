...
void dcacp_hashtable_init(struct inet_hashinfo* hashinfo, unsigned long thash_entries) {
        ...
        inet_hashinfo2_init(hashinfo, "dcacp_listen_portaddr_hash",
...
static void __inet_put_port(struct sock *sk)
{
	...
	dcacp_sk(sk)->icsk_bind_hash = NULL;
...
static void inet_hash2(struct inet_hashinfo *h, struct sock *sk)
{
	...
	if (sk->sk_reuseport && sk->sk_family == AF_INET6)
		hlist_add_tail_rcu(&dcacp_sk(sk)->icsk_listen_portaddr_node,
				   &ilb2->head);
	else
		hlist_add_head_rcu(&dcacp_sk(sk)->icsk_listen_portaddr_node,
				   &ilb2->head);
	ilb2->count++;
...
static void inet_unhash2(struct inet_hashinfo *h, struct sock *sk)
{
	...
	if (!h->lhash2 ||
	    WARN_ON_ONCE(hlist_unhashed(&dcacp_sk(sk)->icsk_listen_portaddr_node)))
		return;

	ilb2 = inet_lhash2_bucket_sk(h, sk);

	spin_lock(&ilb2->lock);
	hlist_del_init_rcu(&dcacp_sk(sk)->icsk_listen_portaddr_node);
	ilb2->count--;
	spin_unlock(&ilb2->lock);
}
...
struct sock *__dcacp_lookup_listener(struct net *net,
...
void dcacp_gen_put(struct sock *sk)
{
	if (!refcount_dec_and_test(&sk->sk_refcnt))
		return;

	// if (sk->sk_state == DCACP_TIME_WAIT)
	// 	inet_twsk_free(inet_twsk(sk));
	// if (sk->sk_state == DCACP_NEW_SYN_RECV)
	// 	reqsk_free(inet_reqsk(sk));
	// else
		sk_free(sk);
}
EXPORT_SYMBOL_GPL(dcacp_gen_put);

void dcacp_edemux(struct sk_buff *skb)
{
	dcacp_gen_put(skb->sk);
}
EXPORT_SYMBOL(dcacp_edemux);
...
EXPORT_SYMBOL_GPL(__dcacp_lookup_established);

/* called with local bh disabled */
static int __dcacp_check_established( struct sock *sk, __u16 lport)
{
	...
	sk_nulls_for_each(sk2, node, &head->chain) {
		...
		if (likely(INET_MATCH(sk2, net, acookie,
					 saddr, daddr, ports, dif, sdif))) {
			// if (sk2->sk_state == DCACP_TIME_WAIT) {
			// 	tw = inet_twsk(sk2);
...
EXPORT_SYMBOL_GPL(dcacp_ehash_nolisten);

static int dcacp_reuseport_add_sock(struct sock *sk,
				   struct inet_listen_hashbucket *ilb)
{
	struct inet_bind_bucket *tb = dcacp_sk(sk)->icsk_bind_hash;
	const struct hlist_nulls_node *node;
	struct sock *sk2;
	...
	sk_nulls_for_each_rcu(sk2, node, &ilb->nulls_head) {
		if (sk2 != sk &&
		    ...
		    dcacp_sk(sk2)->icsk_bind_hash == tb &&
...
int __dcacp_hash(struct sock *sk, struct sock *osk)
{
	struct inet_hashinfo *hashinfo = sk->sk_prot->h.hashinfo;
	struct inet_listen_hashbucket *ilb;
	int err = 0;
...
int dcacp_hash(struct sock *sk)
{
	...
	if (sk->sk_state != TCP_CLOSE) {
		local_bh_disable();
		err = __dcacp_hash(sk, NULL);
		local_bh_enable();
	}

	return err;
}
EXPORT_SYMBOL_GPL(dcacp_hash);

void dcacp_unhash(struct sock *sk)
{
	struct inet_hashinfo *hashinfo = sk->sk_prot->h.hashinfo;
...
EXPORT_SYMBOL_GPL(dcacp_unhash);
...
ok:
	...
	if (sk_unhashed(sk)) {
		...
		// dcacp_ehash_nolisten(sk, (struct sock *)tw);
		dcacp_ehash_nolisten(sk, NULL);
...
int dcacp_hash_connect(struct sock *sk)
{
	...
	return __dcacp_hash_connect(sk, port_offset,
				   __dcacp_check_established);
}
EXPORT_SYMBOL_GPL(dcacp_hash_connect);
