...
def get_oracle_fct(src_addr, dst_addr, flow_size, bandwidth):
    num_hops = 12
    if src_addr / 8 == dst_addr / 8:
        num_hops = 4
    elif src_addr / 64 == dst_addr / 64:
        num_hops = 8
    else:
        num_hops = 12
...
def read_ndp_files(trace, direc = "../../result/ndp/"):
    file = direc + "result_ndp_fat_tree.txt"
    output = {}
...
def read_hpcc_files(trace, direc = "../../result/hpcc/"):
    file = direc + "result_hpcc_load_fat.txt"
    output = {}
    with open(file) as json_file:
        output = json.load(json_file)
    return output
...
def read_homa_aeolus_files(direc = "../../result/homa_aeolus/"):
    file = direc + "result_homa_500_load_100_fat.txt".format(int(bandwidth))
    output = {}
    with open(file) as json_file:
        output = json.load(json_file)
    return output
...
def read_outputs(direc, workload, trace):
    ...
    for i in algos:
        ...
        if i == 'ndp':
            ...
            file = input_prefix  + i +  "_" + trace + ".txt"
