...
then
	...
	while (( flow < NCLIENT ));do
			taskset -c $TASKSET $TARGETDIR/dcPIM/kernel_impl/util/dcpim_test $TARGET:$((4000 + flow)) --pin --sp $(( 10000 * 1 +  flow )) --count 1 dcpimping &
			(( flow++ ))
	done
fi
...
then
	...
	server=0
	NSERVER=15
	ssh $USER\@$TARGETC -t "$TARGETDIR/dcPIM/kernel_impl/util/run_a2a_iperf3.sh dcpim" &
	...
	# done
	sleep 3
	$TARGETDIR/dcPIM/kernel_impl/util/run_a2a_client_iperf3.sh $NCLIENT dcpim
	#flow=0
