...
then
	...
	while (( flow < NCLIENT ));do
			taskset -c $TASKSET /home/qizhe/dcPIM/kernel_impl/util/dcpim_test 192.168.11.125:$((4000 + flow)) --pin --sp $(( 10000 * 1 +  flow )) --count 1 tcpping &
			(( flow++ ))
	done
fi
...
then
	...
	server=0
	NSERVER=15
	ssh jaehyun\@128.84.155.146 -t "/home/qizhe/dcPIM/kernel_impl/util/run_a2a_iperf3.sh tcp" &
	# while (( server < NSERVER ));do
	...
	# done
	sleep 3
	/home/qizhe/dcPIM/kernel_impl/util/run_a2a_client_iperf3.sh $NCLIENT tcp
	#flow=0
...
ssh jaehyun\@128.84.155.146 -t 'sudo trace-cmd clear'
ssh jaehyun\@128.84.155.146 -t 'sudo killall iperf3'
