...
ssh $USER\@$TARGETC -t "sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/dcPIM/kernel_impl/util/pingpong_server --ip $TARGET --port 10000 --iodepth 1 --flowsize $FLOWSIZE --count $NCLIENT --oneside" &
PIDS="$PIDS $!"
		#taskset -c 0 $TARGETDIR/dcpim_kernel/util/server --ip 192.168.10.125 --port $((4000 + core_id))
# 		(( server++ ))
# done
sleep 3
sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/dcPIM/kernel_impl/util/pingpong_client $TARGET:10000  --sp 10000 --count $NCLIENT --iodepth 1 --flowsize $FLOWSIZE --tcp --oneside ping &
