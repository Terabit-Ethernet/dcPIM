...
then
	...
	while (( flow < NCLIENT ));do
			taskset -c $TASKSET /home/qizhe/dcpim_kernel/util/dcpim_test 192.168.10.125:$((4000 + flow)) --sp $(( 10000 * (1) +  flow )) --pin --count 1 tcpping &
			(( flow++ ))
	done
fi
...
then
	...
	# done
	sleep 3
	/home/qizhe/dcpim_kernel/util/run_a2a_client.sh $NCLIENT tcp
fi
