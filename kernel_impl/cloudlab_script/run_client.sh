...
while (( flow < NCLIENT ));do
        ...
        if [[ $SYS == "dcpim" ]]
        then
                        sudo LD_PRELOAD=/home/qizhe/dcPIM/kernel_impl/custom_socket/socket_wrapper.so taskset -c $((core)) iperf3 -c $serverip -p $((dport)) --cport $dport  -t 120 -4  > client_"$serverindex"_"$flow".log &
        else
