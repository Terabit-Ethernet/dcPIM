...
for addr in  "${server_array[@]: 0 : (($num_hosts))}";
	do 
	echo ~/dcPIM/kernel_impl/cloudlab_script/run_server.sh $sys 8 $((serverindex+1))
...
serverindex=0
clientindex=0
for addr in  "${client_array[@]: 0 : (($num_hosts))}";
	do 
		echo $addr ~/dcPIM/kernel_impl/cloudlab_script/run_client.sh $flows $sys $((serverindex+1)) $((clientindex+1+num_hosts))
 		ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa -p 22 $USER@clnode$addr.clemson.cloudlab.us "~/dcPIM/kernel_impl/cloudlab_script/run_client.sh $flows $sys $((serverindex+1)) $((clientindex+1+num_hosts))" & 
		((clientindex++))
		((serverindex++))
	done
sleep 150
