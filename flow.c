...
void init_flow(struct flow* f, uint32_t id, uint32_t size, uint32_t src_addr,
 uint32_t dst_addr, struct ether_addr* ether, uint64_t start_time, int receiver_side){
	f->id = id;
    ...
    f->priority = get_flow_priority(f, params.BDP, params.priority_limit);
