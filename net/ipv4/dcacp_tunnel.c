...
{
	struct sock *sk = sock->sk;
	struct dcacp_tunnel_info ti;

	if (!dev->netdev_ops->ndo_dcacp_tunnel_add ||
	    !(dev->features & NETIF_F_RX_DCACP_TUNNEL_PORT))
		return;
	...
	dev->netdev_ops->ndo_dcacp_tunnel_add(dev, &ti);
}
			     ...
			     unsigned short type)
{
	...
	if (!dev->netdev_ops->ndo_dcacp_tunnel_del ||
	    !(dev->features & NETIF_F_RX_DCACP_TUNNEL_PORT))
		return;
	...
	dev->netdev_ops->ndo_dcacp_tunnel_del(dev, &ti);
}
EXPORT_SYMBOL_GPL(dcacp_tunnel_drop_rx_port);

/* Notify netdevs that DCACP port started listening */
void dcacp_tunnel_notify_add_rx_port(struct socket *sock, unsigned short type)
{
	...
	for_each_netdev_rcu(net, dev) {
		if (!dev->netdev_ops->ndo_dcacp_tunnel_add)
			continue;
		if (!(dev->features & NETIF_F_RX_DCACP_TUNNEL_PORT))
			continue;
		dev->netdev_ops->ndo_dcacp_tunnel_add(dev, &ti);
	}
	rcu_read_unlock();
...
/* Notify netdevs that DCACP port is no more listening */
			 ...
			 bool xnet, bool nocheck)
{
	...
	iptunnel_xmit(sk, rt, skb, src, dst, IPPROTO_DCACP, tos, ttl, df, xnet);
}
