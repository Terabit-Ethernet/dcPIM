 ...
 *
 *		The User Datagram Protocol (DCACP).
 *
 * Authors:	Ross Biro
 ...
 *					for dcacp at least is 'valid'.
 *		Alan Cox	:	Fixed icmp handling properly
 *		Alan Cox	: 	Correct error for oversized datagrams
 *		Alan Cox	:	Tidied select() semantics.
 *		Alan Cox	:	dcacp_err() fixed properly, also now
 *					select and read wake correctly on errors
 *		Alan Cox	:	dcacp_send verify_area moved to avoid mem leak
 *		Alan Cox	:	DCACP can count its memory
 ...
 *		Matt Dillon	:	DCACP length checks.
 *		Alan Cox	:	Smarter af_inet used properly.
 *		Alan Cox	:	Use new kernel side addressing.
 *		Alan Cox	:	Incorrect return on truncated datagram receive.
 *	Arnt Gulbrandsen 	:	New dcacp_send and stuff
 ...
 *	Hirokazu Takahashi	:	HW checksumming for outgoing DCACP
 *					datagrams.
 *	Hirokazu Takahashi	:	sendfile() on DCACP works now.
 *		Arnaldo C. Melo :	convert /proc/net/dcacp to seq_file
...
#include <net/dcacp_tunnel.h>

struct dcacp_table dcacp_table __read_mostly;
...
static int dcacp_lib_lport_inuse(struct net *net, __u16 num,
			       const struct dcacp_hslot *hslot,
...
static int dcacp_reuseport_add_sock(struct sock *sk, struct dcacp_hslot *hslot)
{
...
{
	struct dcacp_hslot *hslot, *hslot2;
	struct dcacp_table *dcacptable = sk->sk_prot->h.dcacp_table;
	int    error = 1;
	struct net *net = sock_net(sk);
	...
	if (!snum) {
		...
		do {
			hslot = dcacp_hashslot(dcacptable, net, first);
		...
		hslot = dcacp_hashslot(dcacptable, net, snum);
		spin_lock_bh(&hslot->lock);
		if (hslot->count > 10) {
			int exist;
			...
			hslot2 = dcacp_hashslot2(dcacptable, slot2);
			if (hslot->count < hslot2->count)
				goto scan_primary_hash;

			exist = dcacp_lib_lport_inuse2(net, snum, hslot2, sk);
			if (!exist && (hash2_nulladdr != slot2)) {
				hslot2 = dcacp_hashslot2(dcacptable, hash2_nulladdr);
...
found:
	...
	if (sk_unhashed(sk)) {
		...
		hslot2 = dcacp_hashslot2(dcacptable, dcacp_sk(sk)->dcacp_portaddr_hash);
		spin_lock(&hslot2->lock);
...
struct sock *__dcacp4_lib_lookup(struct net *net, __be32 saddr,
		...
		int sdif, struct dcacp_table *dcacptable, struct sk_buff *skb)
{
	struct sock *result;
...
int __dcacp4_lib_err(struct sk_buff *skb, u32 info, struct dcacp_table *dcacptable)
{
	struct inet_sock *inet;
...
void dcacp4_hwcsum(struct sk_buff *skb, __be32 src, __be32 dst)
{
	...
	if (!skb_has_frag_list(skb)) {
		...
		uh->check = ~csum_tcpdcacp_magic(src, dst, len,
					       IPPROTO_DCACP, 0);
	} else {
		...
		uh->check = csum_tcpdcacp_magic(src, dst, len, IPPROTO_DCACP, csum);
		if (uh->check == 0)
...
csum_partial:
	...
	uh->check = csum_tcpdcacp_magic(fl4->saddr, fl4->daddr, len,
				      sk->sk_protocol, csum);
	if (uh->check == 0)
		uh->check = CSUM_MANGLED_0;
...
send:
	...
	if (err) {
		if (err == -ENOBUFS && !inet->recverr) {
			DCACP_INC_STATS(sock_net(sk),
				      DCACP_MIB_SNDBUFERRORS, is_dcacplite);
			err = 0;
		}
	} else
		DCACP_INC_STATS(sock_net(sk),
			      DCACP_MIB_OUTDATAGRAMS, is_dcacplite);
	return err;
}
...
int dcacp_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	if (cgroup_bpf_enabled && !connected) {
		err = BPF_CGROUP_RUN_PROG_DCACP4_SENDMSG_LOCK(sk,
					     ...
					     int *total)
{
	...
	while ((skb = skb_peek(rcvq)) != NULL) {
		if (dcacp_lib_checksum_complete(skb)) {
			__DCACP_INC_STATS(sock_net(sk), DCACP_MIB_CSUMERRORS,
					IS_DCACPLITE(sk));
			__DCACP_INC_STATS(sock_net(sk), DCACP_MIB_INERRORS,
					IS_DCACPLITE(sk));
			atomic_inc(&sk->sk_drops);
...
try_again:
	...
	if (unlikely(err)) {
		if (!peeking) {
			atomic_inc(&sk->sk_drops);
			DCACP_INC_STATS(sock_net(sk),
				      DCACP_MIB_INERRORS, is_dcacplite);
		}
		kfree_skb(skb);
		return err;
	}
	...
	if (!peeking)
		DCACP_INC_STATS(sock_net(sk),
			      DCACP_MIB_INDATAGRAMS, is_dcacplite);
	...
	if (sin) {
		...
		*addr_len = sizeof(*sin);
		...
		if (cgroup_bpf_enabled)
			BPF_CGROUP_RUN_PROG_DCACP4_RECVMSG_LOCK(sk,
							(struct sockaddr *)sin);
	}
...
csum_copy_err:
	if (!__sk_queue_drop_skb(sk, &dcacp_sk(sk)->reader_queue, skb, flags,
				 dcacp_skb_destructor)) {
		DCACP_INC_STATS(sock_net(sk), DCACP_MIB_CSUMERRORS, is_dcacplite);
		DCACP_INC_STATS(sock_net(sk), DCACP_MIB_INERRORS, is_dcacplite);
	}
	kfree_skb(skb);
...
void dcacp_lib_unhash(struct sock *sk)
{
	if (sk_hashed(sk)) {
		...
		hslot  = dcacp_hashslot(dcacptable, sock_net(sk),
...
void dcacp_lib_rehash(struct sock *sk, u16 newhash)
{
	if (sk_hashed(sk)) {
		struct dcacp_table *dcacptable = sk->sk_prot->h.dcacp_table;
		struct dcacp_hslot *hslot, *hslot2, *nhslot2;

		hslot2 = dcacp_hashslot2(dcacptable, dcacp_sk(sk)->dcacp_portaddr_hash);
		nhslot2 = dcacp_hashslot2(dcacptable, newhash);
...
static int __dcacp_queue_rcv_skb(struct sock *sk, struct sk_buff *skb)
{
	...
	if (rc < 0) {
		...
		if (rc == -ENOMEM)
			DCACP_INC_STATS(sock_net(sk), DCACP_MIB_RCVBUFERRORS,
					is_dcacplite);
		DCACP_INC_STATS(sock_net(sk), DCACP_MIB_INERRORS, is_dcacplite);
		kfree_skb(skb);
		trace_dcacp_fail_queue_rcv_skb(rc, sk);
		return -1;
	}
...
static int dcacp_queue_rcv_one_skb(struct sock *sk, struct sk_buff *skb)
{
	...
	if (static_branch_unlikely(&dcacp_encap_needed_key) && up->encap_type) {
		...
		if (encap_rcv) {
			...
			if (ret <= 0) {
				__DCACP_INC_STATS(sock_net(sk),
						DCACP_MIB_INDATAGRAMS,
						is_dcacplite);
				return -ret;
			}
		}
...
csum_error:
	__DCACP_INC_STATS(sock_net(sk), DCACP_MIB_CSUMERRORS, is_dcacplite);
drop:
	__DCACP_INC_STATS(sock_net(sk), DCACP_MIB_INERRORS, is_dcacplite);
	atomic_inc(&sk->sk_drops);
	kfree_skb(skb);
	return -1;
}
				    ...
				    int proto)
{
	...
	struct dcacp_hslot *hslot = dcacp_hashslot(dcacptable, net, hnum);
...
start_lookup:
	...
	sk_for_each_entry_offset_rcu(sk, node, &hslot->head, offset) {
		...
		if (unlikely(!nskb)) {
			atomic_inc(&sk->sk_drops);
			__DCACP_INC_STATS(net, DCACP_MIB_RCVBUFERRORS,
					IS_DCACPLITE(sk));
			__DCACP_INC_STATS(net, DCACP_MIB_INERRORS,
					IS_DCACPLITE(sk));
			continue;
		}
	...
	if (first) {
		...
		kfree_skb(skb);
		__DCACP_INC_STATS(net, DCACP_MIB_IGNOREDMULTI,
...
int __dcacp4_lib_rcv(struct sk_buff *skb, struct dcacp_table *dcacptable,
		   int proto)
{
	struct sock *sk;
	struct dcacphdr *uh;
	...
	__DCACP_INC_STATS(net, DCACP_MIB_NOPORTS, proto == IPPROTO_DCACPLITE);
...
csum_error:
	...
	net_dbg_ratelimited("DCACP%s: bad checksum. From %pI4:%u to %pI4:%u ulen %d\n",
			    ...
			    ulen);
	__DCACP_INC_STATS(net, DCACP_MIB_CSUMERRORS, proto == IPPROTO_DCACPLITE);
drop:
	__DCACP_INC_STATS(net, DCACP_MIB_INERRORS, proto == IPPROTO_DCACPLITE);
	kfree_skb(skb);
	return 0;
}
						  ...
						  int dif, int sdif)
{
	...
	struct dcacp_hslot *hslot = &dcacp_table.hash[slot];
					    ...
					    int dif, int sdif)
{
	...
	struct dcacp_hslot *hslot2 = &dcacp_table.hash2[slot2];
		       ...
		       int (*push_pending_frames)(struct sock *))
{
	...
	case DCACP_ENCAP:
		...
		case DCACP_ENCAP_ESPINDCACP_NON_IKE:
			up->encap_rcv = xfrm4_dcacp_encap_rcv;
#endif
			/* FALLTHROUGH */
		case DCACP_ENCAP_L2TPINDCACP:
			up->encap_type = val;
			lock_sock(sk);
			dcacp_tunnel_encap_enable(sk->sk_socket);
			release_sock(sk);
			break;
		default:
			err = -ENOPROTOOPT;
...
struct proto dcacp_prot = {
	...
	.sysctl_wmem_offset	= offsetof(struct net, ipv4.sysctl_dcacp_wmem_min),
	.sysctl_rmem_offset	= offsetof(struct net, ipv4.sysctl_dcacp_rmem_min),
	.obj_size		= sizeof(struct dcacp_sock),
	.h.dcacp_table		= &dcacp_table,
#ifdef CONFIG_COMPAT
...
static struct sock *dcacp_get_first(struct seq_file *seq, int start)
{
	...
	for (state->bucket = start; state->bucket <= afinfo->dcacp_table->mask;
	     ++state->bucket) {
		struct dcacp_hslot *hslot = &afinfo->dcacp_table->hash[state->bucket];
...
void __init dcacp_table_init(struct dcacp_table *table, const char *name)
{
	unsigned int i;

	table->hash = alloc_large_system_hash(name,
					      2 * sizeof(struct dcacp_hslot),
					      uhash_entries,
...
{
	net->ipv4.sysctl_dcacp_rmem_min = SK_MEM_QUANTUM;
	net->ipv4.sysctl_dcacp_wmem_min = SK_MEM_QUANTUM;

#ifdef CONFIG_NET_L3_MASTER_DEV
	net->ipv4.sysctl_dcacp_l3mdev_accept = 0;
#endif
}
